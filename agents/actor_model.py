...
class ActorNetwork:
    ...
    def __init__(self, sess, state_dim, action_dim, action_bound, learning_rate, tau, batch_size):
        ...
        # Op for periodically updating target network with online networkweights
    ...
    def create_actor_network(self):
        inputs = tf.placeholder(dtype=tf.float32, shape=[None, self.s_dim], name='inputs_actor')
        net1 = tf.layers.dense(inputs, units=400,activation=None)
        net2 = tf.layers.batch_normalization(net1)
        net3 = tf.nn.relu(net2)
        net4 = tf.layers.dense(net3, units=300)
        net5 = tf.layers.batch_normalization(net4)
        net6 = tf.nn.relu(net5)
        net_shape = net6.get_shape().as_list()[1]
        shape_for = [net_shape,self.a_dim]
        out = tf.layers.dense(net6,
                              self.a_dim,
